...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_danisman = "SELECT ad_soyad FROM akademisyenler WHERE akademik_id = ?";
    $stmt_danisman = $conn->prepare($sql_danisman);
    $stmt_danisman->bind_param("s", $projeDanismaniID);
    $stmt_danisman->execute();
    $result_danisman = $stmt_danisman->get_result();
    ...
    $sql_ogrenci = "SELECT ogrenci_no FROM ogrenciler WHERE ogrenci_id = ?";
    $stmt_ogrenci = $conn->prepare($sql_ogrenci);
    $stmt_ogrenci->bind_param("s", $ogrenciNo);
    ...
    $sql_update = "UPDATE gliste SET ogrenci_no=?, proje_adi=?, proje_danismani=?, Proje_Süresi_YarıYıl=?, Proje_Kaçıncı_YarıYıl=? WHERE Proje_ID=?";
    $stmt_update = $conn->prepare($sql_update);
    if ($stmt_update === false) {
        echo "Hata: SQL hazırlanamadı.";
        exit();
    }
    $stmt_update->bind_param("ssssss", $ogrenciNo, $projeAdi, $projeDanismani, $projeSuresi, $projeYili, $bitirmeProjeID);
...
<head>
    ...
    <title>Final Sınav Değerlendirme Formu</title>
